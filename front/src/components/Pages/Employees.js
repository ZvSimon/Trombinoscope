...
const Employees = () => {
    ...
    useEffect(() => {
        ...
        Promise.all(initialRequests).then((responses) => {
                ...
                for (const employee of values[0]) {
                    employee.service = values[1].find(s => s.id === employee?.ServicesActiviteId);
                    employee.agence = values[2].find(a => a.id === employee?.AgenceId);
                    employee.direction = values[3].find(d => d.id === employee?.service?.DirectionId);
                    temp.push(employee);
                }
                setEmployees(temp);
            }
  ...
  <Box sx={{
      ...
      p: 3,
        ...
        <Box sx={{display: "flex", alignItems: "center", flexDirection: "column", width: "100%", mx: 5}}>
            ...
            <Grid container spacing={2} mt={1}>
                {employees
                    ?.filter((employee) => {
                        ...
                        ].filter(item => item != null).map(str => str.toLowerCase())
                    ...
                    .map((employee) => (
                        <Grid key={employee.id} item xs={12} sm={6} md={3}>
                            <Card setupdated={setupdated} key={employee.id} employee={employee}/>
                        </Grid>
                    ))}
            </Grid>
        </Box>
